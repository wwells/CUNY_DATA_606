...
__This distribution is unimodal and right skewed.__
...
__This data is also unimodal and right skewed, similar to our population distribution.   The bin from 2500 - 3000 is comparatively bigger, but otherwise, very similar. NOTE:  We used set.seed to ensure that every time the .Rmd file is run, the same sample is taken.__
...
__Our new sample mean is very similar to our samp1 sample mean.  If we took new samples of 100 and 1000, we would expect that the mean of these samples would be even closer to the population mean.__
...
```{r}
m <- round(mean(sample_means50), 2)
```

__Since sample_means50 was created by running a for loop 5,000 times, it has 5000 elements.  The center of the sampling distribution (`r m`) is very close to the center of the population distribution (`r round(mean(area), 2)`).  We would expect the center of distribution created from 50,000 sample means to be even closer to the population mean, although this may be overkill since our population only has 2,930 observations.__
...
for (i in 1:100) {
    s <- sample(area, 50)
    sample_means_small[i] <- mean(s)
}

sample_means_small
```

__Since we ran the for loop 100 times, the vector contains 100 elements.  Each element represents the mean of 50 elements randomly sampled from the area vector.__
...
__When the sample size grows, the center of the sample distribution falls closer to the population mean.   The overall spread or variability also decreases as the frequency with which we pick a sample with a mean closer to the population mean increases.__
...
options(scipen = 999)
set.seed(16)
psamp50 <- sample(price, 50) 
```
__Our point estimate of the population mean based on this sample is `r round(mean(psamp50), 2)`.__
...
for (i in 1:5000) {
    s <- sample(price, 50)
    sample_means50[i] <- mean(s)
}

hist(sample_means50, breaks =20)
```

__The shape of this sampling distribution is nearly normal.  Based on the distribution of these 5000 samples, we would guess that the population mean to be around $180,000, but we can calculate it to be `r round(mean(sample_means50), 0)`. The actual population mean is `r round(mean(price), 0)`, so we can see these are very close.__
...
__This sampling distribution is also nearly normal.   We calculate our point estimate based on this new sample distribution to be `r round(mean(sample_means150), 0)`.  This is comparable to the mean of the distribution with the smaller sample.__
...
__There seems to be less spread or variability in the distribution with the larger sample.   This can be confirmed comparing the SE of each distribution.__

__Let's calculate the SE two ways:  based on the population mean  and based on the standard deviation of our sample distribution. __

*  Sample 50
    * SD of sample distribution = `r round(sd(sample_means50), 2)`
    * SD of population / sqrt(50) = `r round(sd(price)/sqrt(50), 2)`
*  Sample 150 
    * SD of sample distribution = `r round(sd(sample_means150), 2)`
    * SD of population / sqrt(150) = `r round(sd(price)/sqrt(150), 2)`

__We can see that a) these are good approximations for each other and b) that we gain greater potential accuracy by taking a larger sample because it lowers the variability, so long as we continue to ensure sample independance - a threshold generally set to be < 10% of the population.__
