...
conducted in 2000. While there are over 200  variables in this data set, we will work with a small subset.
...
## genhlth <- categorial, ordinal
## exerany <- categorical, nominal
## hlthplan <- categorial, nominal
...
```{r}
# The mosaic plot reveals that the greatest proportion of individuals who 
# have not smoked more than 100 cigarettes are females.
```
...
Next let's consider a new variable that doesn't show up directly in this data set: Body Mass Index (BMI) ([http://en.wikipedia.org/wiki/Body_mass_index](http://en.wikipedia.org/wiki/Body_mass_index)). BMI is a weight to height ratio and can be calculated as:
...
### Here we have plotted the relationship between BMI and respondants who 
# said they had exercised at least once in the last month.  We might expect
# there to be a relationship between the two, but there are so many additional
# contributing factors involved (eg. recently started exercising because BMI
# was getting too high, etc), that this boxplot does not easily help us draw
# conclusions about a relationship between the two.   

### We can say that while there appear to be greater outliers in the BMI of
# those who have exercised in the past month, the IQC seems to show
# considerably less variance and a lower median BMI.    
```
...
distribution, but that shape can change depending on how the data is split between the different bins.  You can control the number of bins by adding an argument to the command. In the next two lines, we first make a default histogram of `bmi` and then one with 50 breaks.
...
median(cdc$weight/cdc$wtdesire)
```

We can see that a person's desired weight is often less than their actual weight.   The mean of the ratio of weight:wtdesire shows ~10% difference. There are some curious outliers skewing the data (actual weight=180, desired weight=680 ?!?!).   The median shows only about a 6% difference.
...
'wdiff' is a discrete numerical variable.   As wdiff approaches 0, the individual's weight and desired weight converge.  If wdiff > 0, the individual's actual weight < wtdesire.   If wdiff < 0 the individual's actual weight > wtdesire.
...
```{r}
summary(wdiff)
boxplot(wdiff, main = "Desired Weight - Actual Weight", ylim = c(-110, 90))
```

NOTE:   Because of the many outliers in the wdiff vector, we are limiting the y axis so the visualization better describes the IQR and whiskers. Without taking this step, the IQR is too squashed to be visually usefull.

Since the 3rd Quartile starts at 0 it suggests that ~75% of the sampled persons have a desired weight under their current weight.  Both the Median and Mean suggest that these people feel like they are 10-15 lbs overweight.  

#### Using numerical summaries and a side-by-side box plot, determine if men tend to view their weight differently than women.

```{r}
boxplot(wdiff~cdc$gender, main="Desired Weight - Actual Weight by Gender", ylim = c(-110, 90))

cdc$wdiff <- wdiff
## get sum stats
summary(cdc$wdiff[which(cdc$gender == 'f')])
sd(cdc$wdiff[which(cdc$gender == 'f')])
summary(cdc$wdiff[which(cdc$gender == 'm')])
sd(cdc$wdiff[which(cdc$gender == 'm')])
```

From the summaries we can determine that while the 3rd Quartile range for both genders is ~0, the majority of the distribution shows greater variability for women, and the median wdiff for women is lower than men. 
...
```{r}
mu <- round(mean(cdc$weight), 2)
sigma <- round(sd(cdc$weight), 2)
OneSd <- subset(cdc, weight <= mu+sigma & weight >= mu-sigma)
prop <- round(nrow(OneSd) / nrow(cdc), 2)
```

The mean of the weight variable is `r mu` and the standard deviation is `r sigma`. For a weight to fall within one sd of the mean, it needs to be in the interval [`r round(mu-sigma, 0)`, `r round(mu+sigma, 0)`].  We subset the data according this interval, and find the proportion by dividing the row length of this subset by the row length of the entire sample.  

`r prop` is the proportion of the sample that have weights that fall within one standard deviation of the mean.  
