...
```{r}
table(kobe_streak)
## The distribution is skewed right, with a baseline of 0.  His longest streak of made baskets was 4.  
```
...
## We used 'set.seed' to ensure the results were consistent each time the .Rmd was knit.   In our simulation, 18 heads, 82 tails. 
```
...
```{r}
x <- replicate(100, sample(outcomes, size = 133, replace = TRUE, prob=c(.45, .55)))
```
