...
```{r}
a <- lm(arbuthnot$year ~ arbuthnot$girls)
a
### we can see visually see there is a trend showing an increase in girls born through the years, and confirm based on the slope of a linear regression fit.  
```
...
```{r}
plot(arbuthnot$year, arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), xlab="Year", ylab="Proportion of Boys")
```
