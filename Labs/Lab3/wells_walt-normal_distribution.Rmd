...
__Both appear to be relatively normal unimodal distributions, with a slightly greater peak width in women's heights than men's.__
...
__Yes.   This distribution looks nearly normal.__
...
__The Q-Q plot of the values simulated from a normal distribution look extremely similar to the Q-Q plot of the female heights Q-Q plot.  As the values trend toward the extremes, they vary slightly from the line in both plots, but the bulk of the distribution stays close to the qqline.__
...
__All the simulated QQ plots look very similar to the one with the actual data.__
...
__The distribution of the fdims$wgt variable is right skewed.   There are some outliers on the higher end.__
...
* What is the probability that a randomly chosen female is shorter than 160 cm?  

```{r}
#theoretical - assuming normal
...
* What is the probability that a randomly chosen female weighs more than 60 Kg? 

```{r}
#theoretical
pnorm(60, mean=mean(fdims$wgt), sd=sd(fdims$wgt), lower.tail = FALSE)
#empirical
sum(fdims$wgt > 60) / length(fdims$hgt)

```

__Not surprisingly, the theoretical and empirical values for height are closer since we determine that female height is distributed nearly normally.  Since female weight is not, these values are far apart.__
...
__It is possible that these values have been rounded or are discrete.   That would account for a stepwise motion in a QQ plot__.
...
__We can see from the QQ plot that the female knee diameter variable is not normally distributed.  It appears to be skewed right.  Our histogram confirms this finding.__ 
