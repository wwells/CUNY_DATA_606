...
```{r}
#  A streak of "0" means a Miss.   | M |
#  A streak of "1" means a Hit then a miss.  | H M |
#  A streak of "2" means two Hits then a miss.   | H H M |
#  And so on...
```
...
```{r}
table(kobe_streak)
## The distribution is skewed right, with a baseline of 0.  
# His longest streak of made baskets was 4.  
```
...
## We used 'set.seed' to ensure the results were consistent each time 
# the .Rmd was knit.   In our simulation, 18 heads, 82 tails. 
```
...
you don't provide a `prob` argument; all elements in the `outcomes` vector have an equal probability of being drawn.
...
```{r}
simstreak <- calc_streak(sim_basket)
table(simstreak)
barplot(table(simstreak))
```

The typical streak length is 0 for 45% shooter.   In this simulation, the longest streak is 6, which occurred only one time.   

#### If you were to run the simulation of the independent shooter a second time, how would you expect its streak distribution to compare to the distribution from the question above? Exactly the same? Somewhat similar? Totally different? Explain your reasoning.

```{r}
sim_basket2 <- sample(outcomes, size = 133, replace = TRUE, prob=c(.45, .55))
simstreak2 <- calc_streak(sim_basket2)
table(simstreak2)
```

Before creating simstreak2, we would guess that a new simulation would generally be very similar to the initial streak distribution.   As the size of the sample goes up, the likelihood of the streak distribution becoming similar follows a normal distribution.    At 133 samples, it is reasonably robust so we would expect general similarities.  

#### How does Kobe Bryant's distribution of streak lengths compare to the  distribution of streak lengths for the simulated shooter? Using this  comparison, do you have evidence that the hot hand model fits Kobe's shooting patterns? Explain.

```{r}
# run simulation
sim <- replicate(10000, calc_streak(sample(outcomes, size = 133, replace = TRUE, prob=c(.45, .55))))
sim_means <- unlist(lapply(sim, mean))
sim_sd <- unlist(lapply(sim, sd))
df <- data.frame(sim_means, sim_sd)

#calculate values
simallmean <- round(mean(sim_means), 3)
simallsd <- round(mean(sim_sd), 3)
mean_kobe_streak <- round(mean(calc_streak(kobe$basket)), 3)
sd_kobe_streak <- round(sd(calc_streak(kobe$basket)), 3)
m <- ecdf(sim_means)
s <- ecdf(sim_sd)

#plot
par(mfrow=c(1,2))
hist(df$sim_means, main="Distribution of Simultation \n Streak Means")
abline(v=mean_kobe_streak, col="red")
hist(df$sim_sd, main="Distribution of Simultation \n Streak SD")
abline(v=sd_kobe_streak, col="red")
```

In order to determine whether the streakiness of Kobe's shooting performance is unusual, we'll run a simulation of a 133 shot series for a 45% shooter 10000 times, and record a summary stat for each series - the Mean and SD of the streak distribution in each of the 10000 simulations.  We can see in the above histogram that the distributions are relatively normal, with some right skew for the rare occasions for longer streaks.   The red lines indicate the mean and SD of Kobe's streak performance.

We calulate the mean value of our simulation means to be `r simallmean`.   We calculate the mean of Kobe's 2009 finals streaks to be `r mean_kobe_streak`.  

We calulate the mean value of our simulation sd to be `r simallsd`.   We calculate the sd of Kobe's 2009 finals streaks to be `r sd_kobe_streak`.   It is reassuring that these are relatively similar. 

We can now use the ecdf function to determine where in our simulation Kobe's performance would fall.   We find that his mean streak performance is only >= to `r m(mean_kobe_streak) * 100`% of our simulations.  This would concretely suggest that his performance does not fit the 'hot hand' model.   
